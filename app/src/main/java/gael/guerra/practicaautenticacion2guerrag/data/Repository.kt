package gael.guerra.practicaautenticacion2guerrag.data

import com.google.firebase.remoteconfig.FirebaseRemoteConfig
import com.google.firebase.ktx.Firebase
import com.google.firebase.remoteconfig.ktx.remoteConfig
import com.google.firebase.remoteconfig.remoteConfigSettings
import gael.guerra.practicaautenticacion2guerrag.CursoFirebaseApp.Companion.context
...
class Repository {
    ...
    suspend fun getMinAllowedVersion(): List<Int> {
        ...
        else minVersion.split(".").mapNotNull { it.toIntOrNull() }
    }

    fun getCurrentVersion(): List<Int> {
        return try {
            val packageInfo = context.packageManager.getPackageInfo(context.packageName, 0)
            packageInfo.versionName?.split(".")?.mapNotNull { it.toIntOrNull() } ?: listOf(0, 0, 0)
        } catch (e: Exception) {
            listOf(0, 0, 0)
        }
    }
